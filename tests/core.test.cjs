...
test('processing mutation is not exposed by the store',t=>{const s=setup(t);assert.equal(s.setState,undefined);});
...
test('Notion recovers an existing row before creating and preserves new revisions',async t=>{
 ...
 await i.syncOrder(o);assert.equal(calls.length,1);assert.ok(calls[0].endpoint.endsWith('/query'));assert.equal(o.notionPageId,'existing');assert.equal(o.notionSyncedRevision,2);assert.equal(o.revision,2);
});
