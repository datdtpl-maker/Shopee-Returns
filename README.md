...
1. Mở **Profile Shopee**, đặt tên tài khoản, chọn **Thêm profile**.
   Bấm **Sửa tên** trên profile, nhập tên mới rồi **Lưu tên**. Giữ nguyên phiên đăng nhập, đơn và trạng thái xử lý. Notion cập nhật bản ghi cũ khi đơn đủ điều kiện đồng bộ; tên cũ được giữ để đối chiếu và không dùng lại cho profile khác.
...
4. Xem mã đơn, profile, mã vận đơn, chữ đỏ/xanh ở cột **Vận chuyển chiều giao hàng**, trạng thái Shopee, kết quả đối chiếu và trạng thái xử lý trong app.
...
5. Khi thực sự đã liên hệ shipper hoặc nhận hàng, bấm nút tương ứng. App lưu ngay và đồng bộ Notion khi kết nối bật. Các nút chỉ cập nhật hệ thống theo dõi, không thực hiện thao tác vận hành trên Shopee.
...
- Chỉ hiển thị, cho thao tác và đồng bộ khi mã đơn vừa thấy trên Shopee khớp đúng **một mã vận đơn hợp lệ** trong Sheet vừa tải. Mã vận đơn lấy từ Sheet của quy trình đóng/gửi hàng; đây không phải xác minh độc lập rằng hãng vận chuyển đã nhận kiện hàng.
...
- Notion dùng bảng **Theo dõi hoàn huỷ Shopee** dưới trang **Quản lý đơn hàng**. Các trường: mã đơn, profile, mã vận đơn, vận chuyển chiều giao hàng, trạng thái Shopee, tình trạng Sheet, kết quả đối chiếu, xử lý và thời gian cập nhật.
...
- Đơn đã có và dữ liệu không đổi: không tạo mới hoặc ghi lại chỉ vì thời gian quét thay đổi. Trạng thái vận chuyển, mã vận đơn hoặc tiến độ xử lý thay đổi: cập nhật bản ghi hiện có. Thời gian trên Notion là lần dữ liệu được ghi; app hiển thị thời gian quét gần nhất.
- Nếu bản ghi đã bị xoá trực tiếp trong Notion, lượt quét mới sẽ kiểm tra và tạo lại một lần. Nếu có nhiều bản ghi cùng profile + mã đơn, app báo lỗi và không tạo thêm.
...
Chưa đóng gói installer, chưa commit/push. Bản chạy trực tiếp dùng `Start.cmd`.
